...
const strategistAsks = ["plan", "advise", "review"] as const

type Json = Record<string, unknown>
type StrategistAsk = (typeof strategistAsks)[number]
...
const strategistSessions = new Map<string, StrategistSession>()
...
function strategistSession(sessionID: string): StrategistSession {
  const current = strategistSessions.get(sessionID)
  if (current) return current
  const next = { consulted: false }
  strategistSessions.set(sessionID, next)
  return next
}

function strategistAsk(prompt: string): StrategistAsk | undefined {
  const value = prompt
    .trimStart()
    .match(/^\{\s*"ask"\s*:\s*"(plan|advise|review)"\s*\}(?:\r?\n|$)/)?.[1]
  return strategistAsks.includes(value as StrategistAsk) ? (value as StrategistAsk) : undefined
}
...
function chargeReadBudget(
  ...
  result.output += exhausted
    ? "\n\n[read budget exhausted — call strategist_state(allocate_read_budget)]"
    : `\n\n[read budget: ${budget.lines}L, ${Math.ceil(budget.chars / 1_000)}K chars left]`
}

export default (async () => ({
  tool: {
    strategist_state: tool({
      ...
      execute: async (args, context) => {
        if (context.agent !== "driver") throw new Error("strategist_state is restricted to driver")
  ...
  "tool.execute.before": async (input, result) => {
    ...
    if (args?.subagent_type !== "strategist") return
    if (!strategistAsk(asString(args.prompt) || "")) {
      throw new Error('strategist-state blocked strategist call: first line must be {"ask":"..."}')
    }
    const current = strategistSession(input.sessionID)
    const requestedTaskID = asString(args.task_id)
    if (!current.task_id && requestedTaskID) {
      throw new Error("strategist-state requires a new strategist task for the first consultation")
    }
    if (!current.task_id || requestedTaskID === current.task_id) return
    throw new Error(`strategist-state requires continuing strategist task_id: ${current.task_id}`)
  },
  ...
  "tool.execute.after": async (input, result) => {
    ...
    const current = strategistSession(input.sessionID)
    const childTaskID = taskID(result.output, result.metadata)
    if (!childTaskID) throw new Error("strategist-state could not observe the strategist task_id")
    if (current.task_id && current.task_id !== childTaskID) {
      throw new Error(`strategist-state observed a different strategist task_id; continue ${current.task_id}`)
    }
    ...
    if (!asString(response?.decision) || !asString(response?.summary)) {
      throw new Error(`strategist-state rejected strategist response; retry with task_id: ${childTaskID}`)
    }

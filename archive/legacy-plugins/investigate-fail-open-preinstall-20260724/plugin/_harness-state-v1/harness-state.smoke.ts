...
let invalidInvestigationBlocked = false
try {
  await start("invalid-investigation", "orchestrator")
  await investigate.execute(investigateArgs, context("invalid-investigation", "orchestrator"))
} catch {
  invalidInvestigationBlocked = true
}
if (!invalidInvestigationBlocked) throw new Error("invalid investigate response was accepted")

...
const agents = new Set(["MetaOrchestrator", "runner"])
const allowedChildren = {
  MetaOrchestrator: new Set(["runner"]),
...
export default (async () => ({
  ...
  "tool.execute.before": async (input, result) => {
    ...
    if (!child) return
    if (!allowedChild(parent, child)) throw new Error(`oc-meta topology blocks ${parent} -> ${child}`)
  },
})) satisfies Plugin

...
- Hand off a bounded outcome, fixed contracts and preserved behavior, implementation freedoms (including which examples are illustrative), edit/resource boundaries, and the decisive acceptance path. Keep it readable rather than imposing a template or a word limit.
...
- Distinguish invariant user/product requirements from experiment fixtures, examples, and time/budget limits. Leave implementation choices open unless the accepted goal requires a restriction; surface any new restriction and its impact on the goal before accepting it.
...
- Own the DAG before substantive delegation: separate user-owned direction choices, uncertain research hypotheses, and implementation with a known direction. Use independently reviewable outcome/hypothesis nodes with inputs, dependencies, observable result, edit scope, protected contracts, and verification. Do not hand an entire epic or several independent goals to one Terra; do not fragment a coherent result by file or by implementation/test/UI role. Replan on a new independent goal or unresolved shared contract rather than inflating a card just because context can be reused.
- Size assignments by the next decision boundary, not file count or wording. If an intermediate result can change the later design, split there; keep implementation, UI/state/persistence, and verification together when they establish one outcome. Reassess scope after initial source evidence. Do not compensate for independent goals, unresolved decisions, or accumulating uncertainty with a longer handoff.
...
- Execute ready independent implementation units in parallel as separate background `terraworker` calls only when source edits, build outputs/static assets, ports/processes, save roots, provider budgets, fixed contracts, and verification cannot interfere. Do not default to serial execution when safe independent work exists.
- Serialize overlapping edits, unresolved shared interfaces, schemas, migrations, lockfiles, generated files, global configuration, shared mutable resources, or work that cannot be independently verified. Do not require unsafe parallelism.
...
- Own requirements satisfaction and final acceptance. Compare the delivered capability with the accepted goal and preserved behavior at the normal user entrypoint, not just a unit test, fixture, diagnostic, or pass count. Review worker evidence before marking a card Done; request only missing report parts when a report is incomplete, and rework with the same worker/task when appropriate. Do not poll background workers.
...
- Request decision-relevant evidence, not a replay: delivered outcome and new restrictions in Changes, decisive user path and failures/counts in Verification, unmet requirements and unclassified failures in Unresolved. Follow up only for missing evidence; do not ask workers to regenerate full reports or re-read already established context.
